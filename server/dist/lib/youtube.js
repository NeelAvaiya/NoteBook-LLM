...
export async function fetchYoutubeTranscript(url) {
    const videoId = url.match(/(?:youtube\.com\/watch\?v=|youtu\.be\/|youtube\.com\/embed\/)([\w-]{11})/)?.[1] ?? url.match(/youtube\.com\/shorts\/([\w-]{11})/)?.[1];
    if (!videoId) {
    ...
    try {
        ...
        throw new ValidationError("Could not fetch transcript. The video may not have captions.");
    }
}
