...
import { deleteSourceVectors, upsertSourceVectors, } from "../lib/pinecone.js";
import { createSourceChunks, deleteChunksBySourceId, findChunksBySourceId, } from "../repository/source-chunk.repository.js";
import { findSourceById, updateSourceRecord, } from "../repository/source.repository.js";
...
export async function markSourceFailed(sourceId, error, existingMetadata) {
...
export async function chunkSourceContent(sourceId, text, pages) {
    ...
    return createSourceChunks(chunks.map((chunk) => ({
        sourceId,
        index: chunk.index,
...
export async function removeSourceFromIndex(workspaceId, sourceId) {
 ...
 *
 */
export async function embedAndIndexSource(source, chunks) {
    const batchSize = 50;
    const records = [];
