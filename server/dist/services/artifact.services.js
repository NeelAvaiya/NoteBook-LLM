...
import { createArtifactRecord, deleteArtifactRecord, findArtifactById, findArtifactByIdAndWorkspaceId, findArtifactsByWorkspaceId, updateArtifactRecord, } from "../repository/artifact.repository.js";
import { NotFoundError } from "../types/app-error.js";
import { gatherSourceContext, generateArtifactContent, } from "./artifact-generation.services.js";
 ...
 *
 */
export async function listArtifactsForWorkspace(workspaceId, userId) {
 ...
 *
 */
export async function getArtifactForWorkspace(workspaceId, artifactId, userId) {
    await getWorkspaceByIdForUser(workspaceId, userId);
    const artifact = await findArtifactByIdAndWorkspaceId(artifactId, workspaceId);
    if (!artifact) {
 ...
 *
 */
export async function createArtifactForWorkspace(workspaceId, userId, input) {
    await getWorkspaceByIdForUser(workspaceId, userId);
    ...
    const artifact = await createArtifactRecord({
        ...
        title: input.title ||
            `${{
                ...
                MINDMAP: "Mind Map",
                REPORT: "AI Report",
            }[input.type]} · ${new Date().toLocaleDateString()}`,
 ...
 *
 */
export async function deleteArtifactForWorkspace(workspaceId, artifactId, userId) {
