...
import { bulkDeleteSourcesForWorkspace, createTextOrMarkdownSource, deleteSourceForWorkspace, getSourceForWorkspace, listSourcesForWorkspace, uploadPdfSource, importWebsiteSource, importYoutubeSource, } from "../services/source.services.js";
...
export async function bulkDeleteSources(req, res) {
    ...
    await bulkDeleteSourcesForWorkspace(workspaceId, req.session.user.id, input.sourceIds);
    res.status(204).send();
}
...
export async function importYoutube(req, res) {
    const { workspaceId } = workspaceIdParamSchema.parse(req.params);
