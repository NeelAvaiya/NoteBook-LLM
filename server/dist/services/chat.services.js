...
import { z } from "zod";
import { convertToModelMessages, createUIMessageStream, isStepCount, pipeUIMessageStreamToResponse, streamText, toUIMessageStream, tool, } from "ai";
...
import { createConversationRecord, findConversationByIdAndWorkspaceId, findConversationsByWorkspaceId, touchConversation, updateConversationRecord, deleteConversationRecord, } from "../repository/conversation.repository.js";
import { createMessageRecord, countMessagesByConversationId, findMessagesByConversationId, } from "../repository/message.repository.js";
import { formatTavilyResultsForPrompt, searchWeb, } from "../lib/tavily.js";
 ...
 *
 */
export async function listConversationsForWorkspace(workspaceId, userId) {
 ...
 *
 */
export async function createConversationForWorkspace(workspaceId, userId, title) {
    await getWorkspaceByIdForUser(workspaceId, userId);
 ...
 *
 */
export async function getConversationMessagesForWorkspace(workspaceId, conversationId, userId) {
    await getWorkspaceByIdForUser(workspaceId, userId);
 ...
 *
 */
export async function deleteConversationForWorkspace(workspaceId, conversationId, userId) {
    await getWorkspaceByIdForUser(workspaceId, userId);
    const conversation = await findConversationByIdAndWorkspaceId(conversationId, workspaceId);
    if (!conversation) {
        throw new NotFoundError("Conversation not found");
 ...
 *
 */
async function resolveConversation(workspaceId, conversationId, firstMessage) {
    if (conversationId) {
        const existing = await findConversationByIdAndWorkspaceId(conversationId, workspaceId);
        if (!existing) {
            throw new NotFoundError("Conversation not found");
        }
        return existing;
 ...
 *
 */
export async function streamWorkspaceChat(res, workspaceId, userId, input) {
    ...
    const contextMessages = conversation.summary &&
